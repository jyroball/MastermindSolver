 ...
 * File:   main.cpp
 * Author: Jyro Jimenez
 * Purpose:  Mastermind Version 3: Basic game
 */
...
int main(int argc, char** argv) {
    ...
    //Make loop for game
    while(game) {
        //Initialize counter for count to zero again
        for(int i = 0; i < 8; i++) {
            count[i] = 0;
        }
        ...
        //HAve count now just out put if they have the same
        for(int i = 0; i < size; i++) {
            if(play[i] == pat[i]) {
                cout << "0";
                //Loop to search for any duplicate
                ...
                //If sattement to take out hints
                if(!same) {
                    count[(play[i] - '0') - 1] = 0;
                }
                else {
        ...
        //Just need to end game when needed
